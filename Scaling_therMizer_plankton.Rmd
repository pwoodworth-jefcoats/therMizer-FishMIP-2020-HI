---
title: "Scaling_therMizer_plankton"
...
The purpose of this script is to test the plankton scaling for the therMizer extension.  Specifically, we want to test using CMIP6-derived resource spectra in place of the mizer resource spectra.  This follows what was done in the previous FishMIP contribution and in [Woodworth-Jefcoats et al. 2019](https://www.frontiersin.org/articles/10.3389/fmars.2019.00383/full).  
...
sim_GFDL_PI_F0 <- project(params_GFDL_PI, initial_n_pp = init_n_pp_GFDL_PIcontrol, t_max = length(times), effort = effort_array_F0)
sim_GFDL_PI_Fvar <- project(params_GFDL_PI, initial_n_pp = init_n_pp_GFDL_PIcontrol, t_max = length(times), effort = effort_array_Fvar)
sim_GFDL_CC585_Fvar <- project(params_GFDL_CC585, initial_n_pp = init_n_pp_GFDL_CC585, t_max = length(times), effort = effort_array_Fvar)

sim_IPSL_PI_F0 <- project(params_IPSL_PI, initial_n_pp = init_n_pp_IPSL_PIcontrol, t_max = length(times), effort = effort_array_F0)
sim_IPSL_PI_Fvar <- project(params_IPSL_PI, initial_n_pp = init_n_pp_IPSL_PIcontrol, t_max = length(times), effort = effort_array_Fvar)
sim_IPSL_CC585_Fvar <- project(params_IPSL_CC585, initial_n_pp = init_n_pp_IPSL_CC585, t_max = length(times), effort = effort_array_Fvar)
...
* Scaling in the same way as the previous FishMIP contribution (slope $\times$ 1.1 and intercept $\times$ 0.9) doesn't seem to work as well this time around.   
* Scaling as in [Woodworth-Jefcoats et al. 2019](https://www.frontiersin.org/articles/10.3389/fmars.2019.00383/full) resulted in a slope that's too steep.    
* Slope $\times$ 1.1 and intercept $\times$ 0.8: Very similar to previous run.  
* Slope $\times$ 1.2 and intercept $\times$ 0.9: Very similar to first attempt, but with somewhat higher predation mortality.  
* Slope unscaled and intercept $\times$ 0.8: Looks very similar to runs with mizer resource dynamics, except with feeding level increasing slightly with body size.  
* Slope unscaled and intercept $\times$ 0.7: Too much.  Not enough food.  Everything dies.
* Slope unscaled and intercept $\times$ 0.85: This is looking better. 
* Slope $\times$ 1.05 and intercept $\times$ 0.85: Nope, changing the slope changes the feeding levels too much.

## Decision

Let's go with leaving slope unscaled and scaling the intercept by 0.85.  We'll combine this with temperature and see how the model performs.